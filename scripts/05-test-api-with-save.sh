...
# API 테스트 스크립트 (결과 저장 버전)
# 모든 쿠버네티스 모니터링 API 엔드포인트를 테스트하고 응답을 표시하며 파일로 저장합니다
...
# 결과 저장 파일명 생성 (현재 시간 포함)
TIMESTAMP=$(date '+%Y-%m-%d-%H-%M-%S')
RESULT_FILE="result/api-test-${TIMESTAMP}.txt"
...
# 헤더 정보를 파일에 기록
log_to_file "=============================================="
log_to_file "Kubernetes 모니터링 API 테스트 결과"
...
call_api() {
    ...
    if [ "$http_code" = "200" ]; then
        ...
        log_to_file "   ✅ 성공 (HTTP $http_code)"
        
        # JSON 응답을 예쁘게 포맷팅 (jq가 설치되어 있는 경우)
        if command -v jq &> /dev/null; then
            formatted_json=$(echo "$response_body" | jq . 2>/dev/null || echo "$response_body")
            echo "$formatted_json"
            log_to_file "$formatted_json"
        else
            echo "$response_body"
            log_to_file "$response_body"
        fi
    else
        echo -e "${RED}   ❌ 실패 (HTTP $http_code)${NC}"
        log_to_file "   ❌ 실패 (HTTP $http_code)"
...
echo -e "${CYAN}========================================${NC}"
log_to_file "========================================"
log_to_file "3️⃣  네임스페이스 기준 API 테스트"
log_to_file "========================================"
...
echo -e "${CYAN}========================================${NC}"
log_to_file "========================================"
log_to_file "4️⃣  디플로이먼트 기준 API 테스트"
log_to_file "========================================"
...
log_to_file "⏰  시계열 조회 추가 테스트 (다른 시간 구간)"
...
echo -e "${CYAN}========================================${NC}"
log_to_file "========================================"
log_to_file "✅ 모든 API 테스트가 완료되었습니다! 🎉"
log_to_file "========================================"
...
log_to_file ""
log_to_file "📋 API 문서 접속:"
log_to_file "   Swagger UI: ${API_BASE_URL}/docs"
log_to_file "   ReDoc: ${API_BASE_URL}/redoc"
...
log_to_file ""
log_to_file "💡 참고사항:"
log_to_file "   - 데이터가 없는 경우 빈 객체 {} 가 반환됩니다"
log_to_file "   - Collector가 메트릭을 수집하기까지 시간이 걸릴 수 있습니다"
log_to_file "   - 시계열 데이터는 window 시간 내의 모든 메트릭을 반환합니다"

echo ""
echo -e "${GREEN}📁 테스트 결과가 저장되었습니다: ${RESULT_FILE}${NC}"
log_to_file ""
log_to_file "📁 테스트 결과 파일: ${RESULT_FILE}"
